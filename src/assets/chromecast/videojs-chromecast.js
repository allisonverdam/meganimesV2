var apiSession, apiConfig, _apiSession;
...
var ChromeCastButton = (function (_Button) {
    ...
    _createClass(ChromeCastButton, [{
        ...
        value: function initializeApi() {
            ...
            appId = this.options_.playerOptions.appId || chrome.cast.media.DEFAULT_MEDIA_RECEIVER_APP_ID;
            sessionRequest = new chrome.cast.SessionRequest(appId);

            if(apiSession)
                apiConfig = new chrome.cast.ApiConfig(sessionRequest, this.sessionJoinedListener(apiSession), this.receiverListener.bind(this));
            else
        ...
        value: function sessionJoinedListener(session) {
            if (session.media.length) {
                ...
                if(session.media[0].media.metadata.title != this.options_.playerOptions.metadata.title){
                    apiSession = session;
                    this.onSessionSuccess(session);
                }else{
                    this.onSessionSuccess(session);
                    apiSession = session;
                }
            ...
            return console.log('Session joined', session);
        }
    }, {
        ...
        value: function doLaunch() {
            _videoJs2['default'].log('Cast video: ' + this.player_.cache_.src + ' Title: ' + this.options_.playerOptions.metadata.title);
        ...
        value: function onSessionSuccess(session) {
            ...
            window.teste = session;

            if(session.media.length){                
                if(session.media[0].media.metadata.title != this.options_.playerOptions.metadata.title){
                    source = this.options_.playerOptions.sources[0].src;
                    type = this.options_.playerOptions.sources[0].type;
                }else{
                    source = this.player_.cache_.src;
            ...
            if (this.options_.playerOptions.metadata) {
                ref = this.options_.playerOptions.metadata;
                for (key in ref) {
                    value = ref[key];
            ...
            //Add poster image on player
                ...
                //Add title on player
                /*
                mediaInfo.metadata.title = this.options_.title;
                mediaInfo.metadata.subtitle = this.options_.subtitle;
                */
                title = this.player().title;
                if (title) {
                    mediaInfo.metadata.title = [titulo];
                }
            ...
            apiSession.loadMedia(loadRequest, this.onMediaDiscovered.bind(this), this.castError.bind(this));
            // apiSession.addUpdateListener(this.onSessionUpdate.bind(this));
        }
    }, {
        ...
        value: function stopCasting() {
            return apiSession.stop(this.onStopAppSuccess.bind(this), this.castError.bind(this));
        }
    }, {
...
var Chromecast = (function (_Tech) {
    ...
    function Chromecast(options, ready) {
        ...
        apiSession = this.options_.source.apiSession;

        this.receiver = apiSession.receiver.friendlyName;
        ...
        apiSession.addUpdateListener(sessionUpdateHanlder);

        this.on('dispose', function () {
            _this.apiMedia.removeUpdateListener(mediaStatusUpdateHandler);
            _apiSession.removeUpdateListener(sessionUpdateHanlder);
    ...
    _createClass(Chromecast, [{
        ...
        value: function update() {
            this.el_.innerHTML = '<div class="casting-image" style="background-size: \'cover\';background-repeat: \'no-repeat\';background-image: url(\'' + this.options_.poster + '\')"></div><div class="casting-overlay"><div class="casting-information"><div class="casting-icon"></div><div class="casting-description"><small>' + this.localize('CASTING TO') + '</small><br>' + this.receiver + '</div></div></div>';
        }
    }, {
        key: 'onSessionUpdate',
        value: function onSessionUpdate(isAlive) {
